...
export default function EmployerDashboardPage() {
  ...
  const [logoPreview, setLogoPreview] = useState(null); // blob preview để hiện ngay, không đợi Cloudinary
            ...
            {section === 'company' && (
              <div className="bg-white border border-gray-200/80 rounded-3xl p-6 sm:p-8 shadow-sm">
                ...
                <div className="mt-6 flex items-center gap-4">
                  {(logoPreview || myCompany?.logo) ? <img src={logoPreview || myCompany.logo} alt="logo" className="w-16 h-16 rounded-2xl object-cover border" /> : <div className="w-16 h-16 rounded-2xl bg-gray-100 border flex items-center justify-center text-gray-400 text-xs">Logo</div>}
                  <label className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-2 bg-white border border-gray-300 rounded-xl shadow-sm cursor-pointer hover:bg-[#f6fbf9] text-gray-700">{logoUploading ? 'Đang tải...' : 'Upload logo'}<input type="file" accept="image/*" className="hidden" disabled={logoUploading} onChange={async (e) => { const f = e.target.files?.[0]; if (!f) return; const blobUrl = URL.createObjectURL(f); setLogoPreview(blobUrl); setLogoUploading(true); try { const r = await uploadCompanyLogo(f); queryClient.setQueryData(['myCompany'], (old) => ({ ...(old || {}), logo: r.url || r.company?.logo })); queryClient.invalidateQueries({ queryKey: ['myCompany'] }); queryClient.invalidateQueries({ queryKey: ['companies'] }); setLogoPreview(null); setCompanyMsg('Upload logo thành công!'); } catch (err) { setLogoPreview(null); setCompanyMsg(err.message); } finally { setLogoUploading(false); e.target.value = ''; } }} /></label>
                </div>
